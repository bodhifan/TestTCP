...
//             }
    ...
    public class CheckImageFactory
    {
        ...
        static string type;
        static public CheckImage GetCheckImage(string type, string name, string pwd)
        {
            //  string type = AccountFactory.getInstance().getImageCodeAcc().PlatformName;
            CheckImageFactory.type = type;
            CheckImageFactory.user = name;
            CheckImageFactory.pwd = pwd;
        ...
        public static void Login()
        {
            do 
            {
                ...
                {
                    checkImg = null;
                    LogHelper.Error("登录 图片验证码失败，重新登录" + ex.StackTrace);
                    Thread.Sleep(1000);
                }
                

            } while (checkImg == null || !checkImg.IsLoginFinshed());
            LogHelper.Info("登录 图片验证码 成功");
